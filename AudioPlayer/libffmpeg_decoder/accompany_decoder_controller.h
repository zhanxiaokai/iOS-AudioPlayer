...
protected:
    ...
    AudioPacket*    currentAccompanyPacket;
    int currentAccompanyPacketCursor;
...
public:
	...
	bool isDecodePausingFlag;
	pthread_mutex_t mDecodePausingLock;
	pthread_cond_t mDecodePausingCondition;
	...
	virtual int readSamples(short* samples, int size);
	...
	virtual float seekToPosition(float position);
